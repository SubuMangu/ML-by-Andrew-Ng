...
#   @output.capture()  # debug
    ...
    def resize_sq(self, bcid):
        ...
        if hasattr(bcid, 'lines') and bcid.lines:
            # Access the first line's properties
            line = bcid.lines[0]
            # Get the current y-data of the line
            ydata = line[0].get_ydata()
            # Calculate the new height (for example, tripling the current height)
            new_height = (ydata[1] - ydata[0]) * 3
            # Set the new y-data for the line to resize it
            line[0].set_ydata([ydata[0], ydata[0] + new_height])
            line[1].set_ydata([ydata[0] + new_height, ydata[0]])
